...
from CMC.special_mapping import SUPPORT_PACKAGE_LIST, FrameworkPackage, TENSOR_MAPPING, API_MAPPING, dataTypeMapping, \
    omitSuffixCall
...
class astTree(ast.AST):
    def __init__(self, root, file, out_dir, possible_modules, unsupport_map, logger, *args, **kwargs):
    ...
    def analyze_ast_node(self):
        ...
        import_nodes = astImportNodes(self.root, self.file, self.import_map, self.unsupport_map, self.logger)
        import_nodes.transform()

        # analyze the requires_grad information
        grad_nodes = astGradApiNodes(self.root, self.file, self.import_map, self.unsupport_map, self.logger)
        ...
        for idx, package in enumerate(node.names):
            ...
            for frameworkPackage in SUPPORT_PACKAGE_LIST[self.sourceDLClass]:
                if re.search(r"{}\.|^{}$".format(re.escape(frameworkPackage), re.escape(frameworkPackage)),
                    ...
                    try:
                        package.name = eval(
                            f'CMC.Matchers.importMatcher.{self.sourceDLClass}_to_{self.targetDLClass}_replace_Import')(
                            package.name, self.sourceDLClass, self.targetDLClass, self.logger)
                        self.success_api_count += 1
                    except Exception as e:
                        log_warning(self.logger,
                                 f"{self.sourceDLClass}_to{self.targetDLClass}_replace_Import is not supported")
...
class astBasicNodes(astBaseNodes):
    def __init__(self, root, file, out_dir, possible_modules, unsupport_map, logger):
    ...
    def get_api_mather(self, source_api):
        if source_api in API_MAPPING.keys():
            api_mapping = API_MAPPING[source_api]
            ...
            if "Matcher" in api_mapping:
                ...
                if self.current_optimizer_lr and "LrschedMatcher" == matcher:
                    return eval(matcher)(self, source_api, api_mapping, self.targetDLClass, self.logger, lr=self.current_optimizer_lr, out_dir=self.out_dir)
                else:
                    return eval(matcher)(self, source_api, api_mapping, self.targetDLClass, self.logger, out_dir=self.out_dir)
        return None


class astGradApiNodes(astBaseNodes):
    def __init__(self, root, file, possible_modules, unsupport_map, logger):
        self.insert_nodes_list = []
        super(astGradApiNodes, self).__init__(root, file, possible_modules, unsupport_map, logger)
