...
class OCSMatcher(BaseMatcher):
    def generate_code(self, kwargs):
        pattern = re.compile(r"""
            (?P<func_call>                                      # Start capturing function calls.
                [a-zA-Z_][\w:]*                                 # Match function names (may include namespaces).
                (?:\s*::\s*|\.)?                                # Optional namespace delimiter (::) or dot (.), to accommodate different calling conventions.
                [a-zA-Z_][\w:]*                                 # Match function names.
            )\s*\(\s*                                           # Match left parenthesis and optional spaces.
            (?P<args>                                           # Start capturing arguments.
                (?:
                    [^()]*?                                     
                    |                                           
                    \((?:                                      
            ...
            \)\s*         |                                        # Match right parenthesis and optional spaces.
           (?P<variable>[a-zA-Z_][\w]*) |                          # variable
           (?P<operator>\+|\-|\*|\/)                               # operator
           ()
        """, re.VERBOSE)
        ...
        for sequences in self.api_mapping["operatorSequence"]:
            ...
            for i, op in enumerate(operation_sequence):
                ...
                sum(1 for op in operation_sequence if op["type"] == "function_call") == 1):
                    continue
                # Retrieve the parameter mapping.
                func_args_mapping = {}
                if 'args' in op:
                ...
                new_ops = [{"value": "return"}]  # Used to store new operations derived from the current op parsing.
                for idx, (k, sequence) in enumerate(sequences["ReturnSequence"].items()):
                    ...
                    if index_of_matching_op is not None:
                        stmt = sequence
                        # Match strings formatted as variable.method<generic>(), retaining key variables.
        ...
        if len(kwargs) != len(sourceDLCLass_param):
            # torch.addcdiv(tensor, value, tansor1, tensor2)
            arg = kwargs.pop(0)
    ...
    def sequenceToCode(self, result_sequence, param_mapping):
        ...
        for i, op in enumerate(result_sequence):
            if op['type'] == "operator":
                # Process the operators in the stack, ensuring to adhere to precedence.
            ...
            # Clear the operator stack and complete the remaining calculations.
