...
def mark_unsupport(code):
    ...
    for i, line in enumerate(lines):
        ...
        for torch_package in SUPPORT_PACKAGE_LIST["pytorch"]:
...
def format_sequential(code):
    ...
    for seq_block in re.finditer(pattern, code):
        ...
        # Handle unnecessary line breaks in the inner_code.
...
class Converter:
    ...
    def run(self):
        ...
        if self.out_dir is None:
            ...
            # Check if out_dir exists, if it does, remove it
            if os.path.exists(self.out_dir):
                if os.path.isdir(self.out_dir):
                    os.makedirs(os.path.dirname(self.out_dir), exist_ok=True)
                else:
                    raise ValueError("The input 'out_dir' must be a directory!")
            else:
                if os.path.isfile(self.out_dir):
                    os.makedirs(os.path.dirname(self.out_dir), exist_ok=True)
                    # If it's a directory, create the directory itself
                else:
                    if self.out_dir.endswith(".py"):
                        raise ValueError("The input 'out_dir' must be a directory!")
                    else:
    ...
    def convert_file(self, old_path, new_path):
        ...
        if old_path.endswith(".py"):
            ...
            fileAst = astTree(root, old_path, self.out_dir, self.possible_modules, self.unsupport_map, self.logger)
