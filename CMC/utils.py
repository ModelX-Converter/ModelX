...
def HandleAliasApi(api, DLClass):
    ...
    #
    # Iterate through the list, using the re.sub method to remove the matching substring
...
def get_modules_and_operators(parent_package):
    ...
    try:
        ...
        print(f"Unable to import module {parent_package}: {e}")
        return []
...
def convert_imports_to_absolute(imports, module_file, project_root=None):
    ...
    for node in imports:
        if isinstance(node, ast.ImportFrom):
            # Generate an absolute import path
            if node.level > 0:
                # Adjust the package path according to relative level
    ...
    if "paddle" in targetDLClass.lower():
        ...
        if "GPU" in backend:
            ...
            if insertion_point in template:
                # Add CUDA compilation options after the 'add_library' call.
