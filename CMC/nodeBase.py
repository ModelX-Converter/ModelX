...
from CMC.utils import CodeFileGenerator, search_pytorch_to_paddle_api, get_operator_parameters, log_info, \
    get_method_source_code_from_string, \
    python_replace_and_persist_modified_methods, c_replace_and_persist_modified_methods, \
    insert_code, get_unique_name, modify_code, detect_backend
...
class BaseMatcher:
    def __init__(self, transformer, source_api, api_mapping, targetDLClass, logger, lr=None, out_dir=None,
                 sourceDLClas="pytorch"):
        self.transformer = transformer
        self.source_api = source_api
        self.sourceDLClass = sourceDLClas
        self.targetDLClass = targetDLClass
    ...
    def get_target_api(self):
        ...
        try:
            ...
            sys.exit(1)
        self.target_api = search_pytorch_to_paddle_api(self.source_api, self.logger)
        return self.target_api
    ...
    def parse_args_and_kwargs(self, api, args, kwargs, tensorCall=False):
        ...
        if api is None:
            return args + kwargs
        self.args, self.kwargs = get_operator_parameters(api, self.logger)
        ...
        elif len(args) + len(kwargs) > len(self.args) + len(self.kwargs):
        ...
        for node in kwargs:
            ...
            if isinstance(node.value, ast.Constant):
                arg_value = str(node.value.value)
            else:
    ...
    def handleVariantCode(self, new_kwargs, res):
        ...
        if not self.target_api in self.persisted_target_apis:
            for param in self.api_mapping['variantCode']:
                if param["param"] in new_kwargs:
                    if ("value" not in param) or (param["value"] in new_kwargs[param["param"]]):
    ...
    def handleCodeGeneration(self, new_kwargs, res):
        ...
        if "x" in new_kwargs and isinstance(new_kwargs["x"], str):
            if self.source_api == "torch.all" or self.source_api == "torch.any":
                ...
                new_kwargs["x"] = "paddle.to_tensor(" + new_kwargs["x"] + ", dtype='float32')"
        ...
        if "y" in new_kwargs and isinstance(new_kwargs["y"], str):
            if self.source_api == "torch.all" or self.source_api == "torch.any":
                ...
                new_kwargs["y"] = "paddle.to_tensor(" + new_kwargs["y"] + ", dtype='float32')"
        ...
        # special parameters
        if any(api in self.source_api for api in ["torch.swapaxes", "torch.transpose", "torch.swapdims"]) and "perm" in new_kwargs:
